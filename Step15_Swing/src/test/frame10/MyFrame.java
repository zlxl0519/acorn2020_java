...
public class MyFrame extends JFrame implements ActionListener, KeyListener{
	...
	public MyFrame() {
		...
		//패 널도 레이아웃을 지정할수 있다( 기본값은 FlowLayout 가운데 정렬이다)
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(command.equals(COMMAND_SEND)) {//전송 버튼을 눌렀을때
			...
			int item=list.getSelectedIndex();// 선탠되지 않으면 -1이라고 뜬다.
			if(item > -1) {// 선택된 cell 이 있을때
				//정말로 삭제 할것인지 물어본다.
				int result=JOptionPane.showConfirmDialog(this, "삭제할거야? 진짜?");
