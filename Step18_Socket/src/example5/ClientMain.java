 ...
 * 	- JSON 예제
 * 	
 * //{ "key" :value } - value 값은 6가지 타입으로 가질수 있다.
 ...
 * 	
 * 	1. 일반 대화 메세지 	
 * 		{"type":"msg", "name" : "김구라", "content" : "안녕하세요"} - JSONObject name 이라는 키값에 "김구라" 가 있는것.
 * 	2.	누군가 입장 했다는 메세지	
 * 		{"type":"enter", "name" : "김구라"}
 * 	3. 누군가 퇴장 했다는 메세지	
 * 		{"type":"out ", "name" : "원숭이"}
...
public class ClientMain extends JFrame implements ActionListener, KeyListener{
	...
	public ClientMain() {
		...
		JScrollPane scroll2=new JScrollPane(jList,
				JScrollPane.VERTICAL_SCROLLBAR_ALWAYS,
				JScrollPane.HORIZONTAL_SCROLLBAR_NEVER);
		//페널에 JList 를 배치하고
		...
		try {
			...
			jsonObj.put("type", "enter");// key 값, value 값
